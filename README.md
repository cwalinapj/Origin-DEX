...
- Active bin (`d = 0`) deposits are forbidden; allocation begins at `d = 1` on each side (V1 rejects instructions attempting `d = 0`).
...
4) applies deterministic rounding: floor each amount, then distribute the remainder to bins with the largest fractional weights.  
   Tie-breaker: when fractional weights tie, allocate to left bins in ascending distance (`d = 1..NL`), then to right bins in ascending distance (`d = 1..NR`).  
